...
public partial class FolderTreeViewControl : UserControl
{
    ...
    public void SetSelectedItem(FileItem item)
    {
        ...
        var rootTreeNode = FindTreeNode(RootFolder);
        if (rootTreeNode != null && !rootTreeNode.IsExpanded)
        {
            TreeView.ExpandNode(rootTreeNode);
            TreeView.UpdateLayout();
        }
        string rootPath = RootFolder.Path;
        string targetPath = target.Path;
        if (targetPath.StartsWith(rootPath, StringComparison.OrdinalIgnoreCase))
        {
            string relativePath = targetPath.Substring(rootPath.Length).TrimStart(Path.DirectorySeparatorChar);
            string[] segments = relativePath.Split(Path.DirectorySeparatorChar);
            FileItem currentItem = RootFolder;
            foreach (var segment in segments)
            {
                FileItem child = currentItem.SubItems?.FirstOrDefault(c =>
